...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Query(value = """
            ...
            WHERE c.enabled = true
              AND (
                   translate(lower(c.name), 'áéíóúñ', 'aeioun') LIKE translate(lower(concat('%', :filter, '%')), 'áéíóúñ', 'aeioun')
    ...
    @Query("SELECT COALESCE(SUM(c.debt),0) FROM Customer c WHERE c.enabled = true")
    Integer getTotalDebt();

    @Query("SELECT COALESCE(AVG(c.debt),0) FROM Customer c WHERE c.enabled = true")
